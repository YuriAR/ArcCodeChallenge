...
class ServerConfigurationsNetworkHandler {
    ...
    fun getGenres(): Single<JSONArray> {
        val path = "/genre/movie/list"
        ...
        return Single.create { subscriber ->
            val request = object : StringRequest(Method.GET, url, { response ->
                Thread({
                    if (!response.isNullOrEmpty()){
                        val responseJson = JSONObject(response)
                        val hasData = responseJson.has("genres")
                        if (hasData){
                            val genreArray = responseJson.getJSONArray("genres")
                            subscriber.onSuccess(genreArray)
                        }
                        else{
                            val throwable = Throwable("No genres found")
                            subscriber.onError(throwable)
                        }
                    }
                    else{
